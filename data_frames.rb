...
class DataFrame
  ...
  def check_input_data(data)
    if data.class == Array
      data.each do |row|
        ...
        if row.class != Hash
          raise Exception.new("Input data-array contains non-hash values (#{row.class} provided).")
        else
      ...
      data.keys.each do |col|
        ...
        if data[col].class != Array
          raise Exception.new("Input data-hash contains non-array values (#{data[col].class} provided for column '#{col}').")
        else
  ...
  def check_col_name(col_name)
    ...
    if !valid_header_classes.include?(col_name.class)
      raise Exception.new("Invalid header class '#{col_name.class}' detected. Headers can only be from the following classes: '#{valid_header_classes.join(', ')}'.")
    end
  end
  ...
  # Check the input array to make sure all of the inputs are of the same class
  def check_data_consistency(data)
    data_classes = data.map {|v| v.class}.uniq - [NilClass]
    if data_classes.length > 1
      raise Exception.new("Multiple non-nil classes detected in input data: #{data_classes.join(', ')}")
    elsif data_classes.length == 1
      data_classes.first
    elsif data_classes.length == 0
      NilClass
    end
  end
  ...
  def head(df_length=10)
    if nrows < df_length
      puts to_s(limit: nrows)
    else
      puts to_s(limit: df_length)
    end
  end

  # Returns a string containing the DataFrame as a tab-delimitted tableData
  def to_tab
    tab_data = columns.join("\t") + "\n"
    to_a.each {|r| tab_data += r.values.join("\t") + "\n"}
    tab_data
  end

  # Returns the number of rows in the DataFrane
  def nrows
    @df.values.first.length
  end
  ...
  def select(&block)
    DataFrame.new(to_a.select(&block))
  end
  ...
  def sample(n)
    DataFrame.new(to_a.sample(n))
  end

  # Check to see whether the specified object is an array. If it is not, raise an exception.
  def check_array(object)
    if object.class != Array
      raise ArgumentError.new("Specified object must be an array (#{object.class} was input).")
    end
  end

  # Check to make sure the input array is the same length as the DataFrame
  def check_input_col_length(arr)
    if arr.length != nrows
      raise ArgumentError.new("The length of the column provided does not match the lengths of the other columns in the DataFrame.")
    end
  end
  ...
  def check_if_col_names(array)
    ...
    array.each do |col_name|
      if !@df.keys.include?(col_name)
        raise ArgumentError.new("Column name '#{col_name.to_s}' not found in DataFrame.")
      end
    end
  end
  ...
  # NOTE:  base Ruby has a one-line built-in method for doing this with arrays--better to use that
  def select_rows(select_array)
    check_array(select_array)
    raise ArgumentError.new('Input array contains non-Boolean (true/false) values.') if (select_array.uniq - [true, false]).length > 0
  ...
  def add_column(col_name, col_data)
    ...
    # Check the length of the column to add to make sure it is the same length as the DataFrame
    check_input_col_length(col_data)
    # Ensure that the column data values are all of the same class
    dclass = check_data_consistency(col_data)
    @df[col_name] = col_data
    # Get the data type of the new column that was added
    @dtypes[col_name] = dclass
  end
  ...
  def remove_column(col_name)
    ...
    @df.keys.include?(col_name) ? col_name : nil
  end
  ...
  def convert(col, type)
    if ![String, Fixnum, Float, Date, DateTime, NilClass].include?(type)
      raise ArgumentError.new("Convert does not support the specified data type: #{type}")
    end
    if col.class == Array
      check_if_col_names(col)
  ...
  def change_values(col, val, select_array)
    ...
    if val.class != @dtypes[col]
      raise Exception.new("The value to be changed to in the column (#{val}) is of class #{val.class}, whereas the column (#{col}) data type is #{@dtypes[col]}")
    end
    @df[col].each_index {|i| @df[col][i] = val if select_array[i]}
  end

  # Change the column data to be the specified array
  def change_col_data(col, data)
    check_col_name(col)
    check_if_col_exists(col)
    check_array(data)
    check_input_col_length(data)
    dclass = check_data_consistency(data)
    @df[col] = data
    @dtypes[col] = dclass
  end
      ...
      for i in 0...@df[@df.keys[0]].length
        ...
        if match
          ...
          for j in 0...other_df.nrows
            other_match = true
            combo.keys.each {|col| other_match = false if other_df.column(col)[j] != combo[col]}
            if other_match
              ...
              other_df.columns.each do |col|
                if !combo.keys.include?(col) && !@df.keys.include?(col)
                  new_df[col.to_s].push(other_df.column(col)[j])
                elsif !combo.keys.include?(col) && @df.keys.include?(col)
                  new_df[col.to_s + lsuffix.to_s].push(other_df.column(col)[j])
                end
              end
            end
          end
  ...
  def append(other_df, inplace: false)
    ...
    if @dtypes == other_df.dtypes(print_types: false)
      if inplace
        ...
        other_df.columns.each {|col| @df[col] += other_df.column(col)}
        @df
      else
        # Create a new DataFrame, append the rows, and return
        data_arr = (0...nrows).each_with_object([]) do |i, data_arr|
  ...
  # Return value counts of all uniq values in the specified column
  def value_counts(col, normalize: false, show: true, include_nil: true)
    check_if_col_exists(col)
    puts '-' * 50 if show
    if include_nil
      val_counts = @df[col].uniq.each_with_object({}) do |val, h|
        h[val] = @df[col].count(val)
        puts "#{val}#{' ' * (30 - val.to_s.length)}#{@df[col].count(val)}" if show
      end
    else
      val_counts = @df[col].uniq.each_with_object({}) do |val, h|
        h[val] = @df[col].count(val) if val != nil
        puts "#{val}#{' ' * (30 - val.to_s.length)}#{@df[col].count(val)}" if show && val != nil
      end
    end
    puts '-' * 50 if show
    val_counts
  ...
  # Ensure that the specified column is either of type Fixnum or Float
  def check_if_numeric(col)
    if ![Fixnum, Float].include?(@dtypes[col])
      raise Exception.new("Specified column '#{col}' is not of a numeric data type (Float, Fixnum)--#{@dtypes[col]} was provided.")
    end
  end

  # Calculate the mean of the specified numeric data type column
  def mean(col)
    check_if_col_exists(col)
    check_if_numeric(col)
    @df[col].inject(:+).to_f / nrows
  end

  # Calculate the standard deviation of the SAMPLE of the specified numeric data type column
  def std(col)
    xbar = mean(col)
    tot = 0
    @df[col].each {|v| tot += (v - xbar)**2}
    Math.sqrt(tot / (nrows - 1))
  end
  ...
  def median(col)
    ...
    if nrows%2 == 1
      ...
      sorted_col = @df[col].sort
      (sorted_col.sort[nrows / 2] + sorted_col.sort[nrows / 2 - 1]) / 2.0
    end
  end

  # Calculate the standard deviation of the POPULATION of the specified numeric data type column
  def stdp(col)
    mu = mean(col)
    tot = 0
    @df[col].each {|v| tot += (v - mu)**2}
    Math.sqrt(tot / nrows)
  end
